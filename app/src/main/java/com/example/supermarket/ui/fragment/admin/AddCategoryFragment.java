...
import com.example.supermarket.databinding.FragmentAddCategoryBinding;
...
public class AddCategoryFragment extends Fragment {
    ...
    private void inputData() {
        category = binding.edtAddCategory.getText().toString().trim();

        if (TextUtils.isEmpty(category)) {
            binding.edtAddCategory.setError("Enter Category Name");
            return;
        }

        if (image_uri == null) {
            Toast.makeText(getContext(), "Add Category Image", Toast.LENGTH_SHORT).show();
            return;
        }
        check(category);
    ...
    private void check(String category) {
        progressDialog.setTitle("Checking Category...");
        progressDialog.show();
        ...
        storageReference.putFile(image_uri).addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
            ...
            public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                ...
                if (uriTask.isSuccessful()) {
                    CategoryModel categoryModel = new CategoryModel(category, downloadImageUri.toString(), timestamp);

                    //save to db

                    DatabaseReference ref = FirebaseDatabase.getInstance().getReference().child("Category");
                    ref.child(timestamp).setValue(categoryModel);
...
//        startActivity(new Intent(getActivity(), MainUserActivity.class));
        ...
        binding.imgCategory.setImageResource(R.drawable.ic_add_photo_light_gray);
        image_uri = null;
    }
    ...
    public void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        if (resultCode == RESULT_OK) {
            ...
            if (requestCode == IMAGE_PICK_GALLERY_CODE) {
                ...
                //set to imageview
                binding.imgCategory.setImageURI(image_uri);
